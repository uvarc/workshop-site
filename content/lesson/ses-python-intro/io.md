...
<class 'float'>
```
You can see that the unconverted input is of the type **string**.  By passing the input to `float()` in line 3 we are converting the data type to a **float** (real number with decimals).  We call this operation a type cast.
...
print ("Continuing on last line.")
```
**Output:**
```
First line.
Second line.
Third line. Continuing on last line.
```
...
The third line prints an f-string which evaluates the contents of the curly braces when the program is run.  That means that expressions can be used inside the curly braces, in this case we use the variable `n` to define the number of decimal places rather than hardcoding the value. You can also use capital `F` instead of `f` and single quotation marks (`'`) instead of double quotes (`"`).  So this would be a valid alternative for our example:
```
print(F'Pi to {n} places is {math.pi:.{n}f}')
```
...
Let's limit the output to four decimal places (remember n=12):
```
...
We can now process through the list using a loop.  Note that we use `\r\n` to make sure we accommodate Windows, Mac OSX, and Linux operating systems.  An alternative is to use the `str.splitlines()` method which splits lines whether they end in `\r\n`, in `\r` or in `\n` (unless instructed otherwise).  

Yet another option for reading text files is to use a Python idiom in which we iterate through the file using the file identifier.  Suppose we have a file with comma-separated values.  We can read the data with:
```
fin = open("filename")
for line in fin:
    data = line.rstrip("\r\n").split(",")
...
When we have completed this loop x will contain the values from the first column and y will contain values from the third column.  In this example we are not interested in the other columns.  We also convert on the fly to float.  This example assumes that our data file has one line of text header, which we skip by calling `readline` before the for loop.
...
To write a file it must have been opened appropriately.  We can use print with the addition of a `file=` argument. Note the use of f-string for formatting.

```
fout = open("myfile", "w")
print(f"Format string {x:.5f}, {y:.3f}",file=fout)
fout.close()
```
...
try:
    ...
    print ("The input parameter cannot be converted to a float value.)
```
...
The examples above stop on the named exceptions `ValueError` or `IOError` which catch any errors with casting values or the file input/output operations, respectively.  Any other exception will not be caught.  It is not necessary to name exceptions; the following will be executed for any exception.
...
<p>
Use Spyder to write a program that converts a measurement from inches to centimeters (1 inch = 2.54 cm).  The program should request from the user the measurement in inches, compute the conversion and output the converted measurement in inches.  Remember that you will need to convert from strings.   
</p>
</details>
...
<p>
1. Open a file <code>data.txt</code> in write mode.  Write to the file three columns of numbers separated by commas.  These columns should be <code>n</code>, <code>n<sup>2</code> and <code>n<sup>3</code> for <code>n</code> going from 1 to 20 for a total of 20 rows. <br> 
2. Read back the file line by line. Split each line and store the elements in a list.  Use these list values to compute and print <code>a+bn+cn<sup>2</sup>+dn<sup>3</sup></code> for <code>a=1.</code>, <code>b=2.4</code>, <code>c=5.8</code>, <code>d=0.7</code>.<br>
